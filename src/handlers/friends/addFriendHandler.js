...
const addFriendHandler = async (req, res) => {
  try {
    ...
    if (validateAdd) { return res.status(200).json({message: "The friend has been added"})};
    res.status(401).json({error: "An error has occurred, check the parameters you entered"})
  } catch (error) {
